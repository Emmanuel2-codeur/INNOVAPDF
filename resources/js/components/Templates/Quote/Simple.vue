            ...
            <table class="w-full text-left text-xs mb-8 border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="item in doc.invoiceItems" :key="item.id">
                        ...
                        <td class="py-3 text-right text-gray-600">{{ item.unitPrice.toLocaleString('fr-FR') }} {{ currency }}</td>
            ...
            <div class="flex justify-end mb-8">
                <div class="w-1/2 space-y-2 text-xs border-t pt-4">
                    <div class="flex justify-between text-gray-600">
                        ...
                        <span class="font-medium text-gray-900">{{ subtotal.toLocaleString('fr-FR') }} {{ currency }}</span>
                    </div>
                    <div class="flex justify-between text-gray-600">
                        <span>TVA ({{ Math.round(taxRate * 1000) / 10 }}%) :</span>
                        <span class="font-medium text-gray-900">{{ taxAmount.toLocaleString('fr-FR') }} {{ currency }}</span>
                    </div>
